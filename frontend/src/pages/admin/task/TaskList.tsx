...
export default function TaskList() {
    ...
    const fetchTasks = async (page: number, search: string) => {
        try {
            ...
            const res = await api.get(`/tasks?page=${page}&limit=5&search=${search}&status=${status}`)
            console.log("res -->", res)
            ...
            <Card className="relative border-slate-800 bg-slate-900/50 backdrop-blur-xl shadow-2xl overflow-hidden group">
                            ...
                            <Table>
                                <TableHeader className="bg-slate-950/50">
                                    <TableRow className="border-slate-800 hover:bg-transparent">
                                        <TableHead className="text-slate-400 font-medium h-12">Sr.No.</TableHead>
                ...
                <CardFooter className="w-full flex justify-end items-center gap-2">
                    <Button
                        ...
                        disabled={page <= 1}
                        className="bg-slate-600 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                    ...
                    <Button
                        variant="outline"
                        className="cursor-default border-slate-400 text-slate-700"
                        disabled
                    >
                        {page} / {totalPages}
                    </Button>
